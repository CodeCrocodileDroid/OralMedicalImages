...
batch_size = 16
epochs = 50
num_sample_images = 5  # Number of sample images to save

# ============================================================
# Preprocessing: Gabor Filter Conversion
# ============================================================
def gabor_preprocess(img):
    if img.shape[-1] == 1:
    ...
    return filtered_rgb / 255.0

# ============================================================
# Save Sample Images (Original and Gabor-Filtered)
# ============================================================
def save_sample_images(test_gen, run_dir, num_images=5):
    sample_dir = os.path.join(run_dir, "sample_images")
    os.makedirs(sample_dir, exist_ok=True)
    print(f"Saving sample images to: {sample_dir}")
    x_batch, _ = next(test_gen)
    for i in range(min(num_images, len(x_batch))):
        img = x_batch[i]
        # Save original image (before preprocessing)
        original_img = np.uint8(img * 255)  # Scale back to 0-255
        original_path = os.path.join(sample_dir, f"original_{i}.png")
        try:
            cv2.imwrite(original_path, cv2.cvtColor(original_img, cv2.COLOR_RGB2BGR))
            print(f"Saved original image: {original_path}")
        except Exception as e:
            print(f"Error saving original image {original_path}: {e}")
        
        # Save Gabor-filtered image
        gabor_img = gabor_preprocess(img)
        gabor_img = np.uint8(gabor_img * 255)  # Scale back to 0-255
        gabor_path = os.path.join(sample_dir, f"gabor_{i}.png")
        try:
            cv2.imwrite(gabor_path, cv2.cvtColor(gabor_img, cv2.COLOR_RGB2BGR))
            print(f"Saved Gabor-filtered image: {gabor_path}")
        except Exception as e:
            print(f"Error saving Gabor-filtered image {gabor_path}: {e}")

# ============================================================
# Data Generators
# ============================================================
datagen = ImageDataGenerator(preprocessing_function=gabor_preprocess)
...
def make_gradcam_heatmap(img_array, model, last_conv_layer_name, pred_index=None):
    ...
    return heatmap  # Already a NumPy array
...
def save_gradcam_images(test_gen, model, run_dir, num_images=5):
    ...
    for i in range(min(num_images, len(x_batch))):
        ...
        gradcam_path = os.path.join(gradcam_dir, f"gradcam_{i}.png")
        try:
            cv2.imwrite(gradcam_path, superimposed_img)
            print(f"Saved Grad-CAM image: {gradcam_path}")
        except Exception as e:
            print(f"Error saving Grad-CAM image {gradcam_path}: {e}")
...
def save_roc_curves(y_true, y_pred_probs, run_dir, class_labels):
    ...
    try:
        ...
        print(f"Error saving ROC curve {roc_path}: {e}")
    plt.close()
        ...
        for dropout_rate in param_grid["dropout_rate"]:
            for lr in param_grid["lr"]:
                for l2_lambda in param_grid["l2_lambda"]:
                    ...
                    print(f"History log path: {history_log_path}")
                    print(f"Training curves path: {plot_path}")
                    print(f"Confusion matrix path: {cm_path}")
                    print(f"Classification report path: {report_path}")
                    ...
                    try:
                        ...
                        print(f"Error saving final model to {final_model_path}: {e}")
                        continue
                    ...
                    try:
                        ...
                        print(f"Error saving training curves to {plot_path}: {e}")
                    plt.close()
                    ...
                    try:
                        ...
                        print(f"Error saving confusion matrix to {cm_path}: {e}")
                    ...
                    try:
                        ...
                        print(f"Error saving classification report to {report_path}: {e}")

                    # Save ROC curves
                    save_roc_curves(y_true, y_pred_probs, run_dir, class_labels)

                    # Save Grad-CAM samples
                    save_gradcam_images(test_gen, model, run_dir, num_images=num_sample_images)

                    # Save sample images (original and Gabor-filtered)
                    save_sample_images(test_gen, run_dir, num_images=num_sample_images)
                    ...
                    try:
                        ...
                        print(f"Error saving results to {results_csv_path}: {e}")