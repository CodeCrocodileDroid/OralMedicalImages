...
# Setup training policy - Fixed: Force float32 for older GPUs to avoid OOM and compatibility issues
if len(gpus) > 0:
    try:
        for gpu in gpus:
            tf.config.experimental.set_memory_growth(gpu, True)
        tf.keras.mixed_precision.set_global_policy('float32')  # Force float32 for GTX 1050 (compute 6.1)
...
# ============================================================================
# CONFIGURATION - OPTIMIZED FOR LOW MEMORY GPU (GTX 1050)
# ============================================================================

IMG_HEIGHT = 128  # Reduced from 224 for memory
IMG_WIDTH = 128   # Reduced from 224 for memory
PATCH_SIZE = 8    # Reduced from 16 for memory
NUM_PATCHES = (IMG_HEIGHT // PATCH_SIZE) * (IMG_WIDTH // PATCH_SIZE)  # 256 (was 196)
PROJECTION_DIM = 384  # Reduced from 768
NUM_HEADS = 6      # Reduced from 12
TRANSFORMER_LAYERS = 6  # Reduced from 12
MLP_HEAD_UNITS = [512, 256]  # Reduced from [2048, 1024]
...
print(f"\n📊 Enhanced Configuration (Memory-Optimized):")
...
print("🔧 Memory Optimizations: Smaller model, batch size, image size for GTX 1050")
...
run_name = f"vit_run_{timestamp}_optimized"
...
class TransformerBlock(layers.Layer):
    ...
    def __init__(self, embed_dim, num_heads, mlp_dim, **kwargs):
        ...
        self.embed_dim = embed_dim  # Fixed: Store as attribute
        self.num_heads = num_heads  # Fixed: Store as attribute
        self.mlp_dim = mlp_dim  # Fixed: Store as attribute
...
print(f"\n================================================================================\n📋 LOCAL SETUP CHECKLIST\n================================================================================\n✅ Dataset path: {base_data_path}\n✅ Outputs saved in: {run_dir}\n✅ Enable GPU if available\n⏱️ Expected training time: 1-2 hours (optimized)\n💾 Memory required: ~1GB GPU\n🎯 Expected accuracy: >95% with L2 regularization\n================================================================================\n")
